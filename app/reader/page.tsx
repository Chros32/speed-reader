...
export default function ReaderPage() {
  ...
  const words = useMemo(() => text ? parseTextToWords(text) : [], [text]);
  ...
  // Handle successful payment redirect (URL params are external state)
  ...
  useEffect(() => {
    if (!showSuccessMessage || isPremium) return;

    const params = new URLSearchParams(window.location.search);
    const sessionId = params.get('session_id');
    const plan = params.get('plan');
    const durationDays = plan === 'lifetime' ? 36500 : 365;
    ...
    if (sessionId) {
      fetch(`/api/checkout/session?session_id=${sessionId}`)
        ...
        .then(data => {
          if (data.customerId) {
            activatePremium(durationDays, data.customerId);
          } else {
            activatePremium(durationDays);
          }
        })
        .catch(() => {
          activatePremium(durationDays);
  ...
  }, [showSuccessMessage, activatePremium, isPremium]);
  ...
  const handleTextSubmit = useCallback((newText: string) => {
    ...
    const doc = saveDocument(newText);
